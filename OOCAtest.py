...
# credentials from env file, no hardcoded passwords in repo
load_dotenv()
...
# connect without database first to allow clean schema creation on any machine
def db_ini():
    con = sq.connect(host=db_host, user=db_user, password=db_password)
...
def weather():
    ...
    con = sq.connect(host=db_host, user=db_user, password=db_password, database=db_name)
    curs = con.cursor()
...
#this is for each city's URL so that we don't have to have 5 long lines of codes for the 5 different cities
    ...
    #I use try except to prevent half-filled rows
        ...
        try: 
            ...
            resp.raise_for_status() #checks status if OK, if not then go except (Addressing Point 1: added () so it actually executes)
        ...
        # Addressing Point 1 & 2: Catch Key/Value errors and pass a single tuple (city, str(err)) to append()
        except (rq.exceptions.RequestException, KeyError, ValueError) as err:
